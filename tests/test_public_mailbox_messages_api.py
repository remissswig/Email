...
class PublicMailboxMessageSearchTests(unittest.TestCase):
    def setUp(self):
        web_outlook_app.clear_public_mailbox_result_cache()
        self.account = {
            'id': 7,
    ...
    def test_outlook_accounts_use_graph_recipient_search_and_recent_scan_without_detail_refetch(self):
        account = {
            **self.account,
            'client_id': 'client-id',
    ...
    def test_outlook_recent_scan_can_supersede_stale_graph_recipient_search_result(self):
        account = {
            **self.account,
            'client_id': 'client-id',
            'refresh_token': 'refresh-token',
        ...
        stale_graph = {
            **self.item('graph-stale', 'target@example.com', '2026-08-21T12:00:00Z'),
            '_detail': {
                'id': 'graph-stale',
                'subject': 'Stale graph result',
                'from': 'sender@example.com',
                'to': 'target@example.com',
                'date': '2026-08-21T12:00:00Z',
                'body': '<p>stale body</p>',
        ...
        newer_scan = self.item('scan-newer', 'target@example.com', '2026-08-21T12:03:00Z')
        ...
        with patch.object(
            ...
            web_outlook_app,
            'fetch_email_detail_for_account',
            return_value=self.detail(newer_scan, body='<p>newer body</p>'),
        ) as detail_mock:
            result = web_outlook_app.find_public_mailbox_messages(
                account,
                'target@example.com',

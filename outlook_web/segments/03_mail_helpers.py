...
def is_timeout_like_exception(exc: Exception) -> bool:
    error_type = type(exc).__name__.lower()
    error_details = str(exc).lower()
    return (
        isinstance(exc, (TimeoutError, requests.exceptions.Timeout))
        or 'timeout' in error_type
        or 'timed out' in error_details
    )


def build_email_detail_exception_result(exc: Exception) -> Dict[str, Any]:
...
def get_email_detail_graph(client_id: str, refresh_token: str, message_id: str, proxy_url: str = None,
                           fallback_proxy_urls: Optional[List[str]] = None,
                           structured_error: bool = False) -> Optional[Dict]:
    """使用 Graph API 获取邮件详情"""
    access_token = get_access_token_graph(client_id, refresh_token, proxy_url, fallback_proxy_urls)
    if not access_token:
        return None
    ...
    try:
        ...
        if structured_error:
            return build_email_detail_exception_result(exc)
        return None
...
def search_imap_message_ids_by_recipient(mail, recipient: str) -> tuple[Optional[List[bytes]], str, List[Dict[str, Any]]]:
    attempts: List[Dict[str, Any]] = []
    recipient_text = str(recipient or '')

    try:
        status, data = mail.uid('SEARCH', None, 'HEADER', 'TO', recipient_text)
        attempts.append({
            'mode': 'uid',
            'status': str(status),
            'response': sanitize_error_details(str(data or ''))[:200],
    ...
    try:
        status, data = mail.search(None, 'HEADER', 'TO', recipient_text)
        attempts.append({
            'mode': 'sequence',
            'status': str(status),
            'response': sanitize_error_details(str(data or ''))[:200],
        ...
        if status == 'OK':
            payload = data[0] if data else b''
            return payload.split() if payload else [], 'sequence', attempts
        if str(status) in {'NO', 'BAD'}:
            return None, 'unsupported', attempts
        return None, '', attempts
    except Exception as exc:
        attempts.append({
            'mode': 'sequence',
...
IMAP_RECIPIENT_HEADER_BATCH_SIZE = 50
IMAP_RECIPIENT_HEADER_FETCH_QUERY = '(UID INTERNALDATE BODY.PEEK[HEADER.FIELDS (TO SUBJECT FROM DATE)])'
...
def parse_imap_header_batch_fetch_response(data: Any) -> tuple[List[Dict[str, Any]], str]:
    ...
    for item in items:
        ...
        if isinstance(item, tuple):
            tuple_items = list(item)
            head = tuple_items[0] if tuple_items else None
            if isinstance(head, memoryview):
                head = head.tobytes()
            head_text = ''
            if isinstance(head, (bytes, bytearray)):
                head_text = head.decode('utf-8', errors='ignore')
                response_fragments.append(head_text)
            elif head is not None:
                head_text = str(head)
                response_fragments.append(head_text)
            ...
            for payload in tuple_items[1:]:
                if isinstance(payload, memoryview):
                    payload = payload.tobytes()
                if raw_header is None and isinstance(payload, (bytes, bytearray)) and payload:
                    raw_header = bytes(payload)
                    continue
                if isinstance(payload, (bytes, bytearray)):
                    response_fragments.append(payload.decode('utf-8', errors='ignore'))
                elif payload is not None:
            ...
            sequence_match = re.search(r'^\s*(\d+)', head_text)
            uid_match = re.search(r'\bUID\s+(\d+)\b', head_text, flags=re.IGNORECASE)
            records.append({
                'sequence_id': sequence_match.group(1) if sequence_match else '',
                'uid': uid_match.group(1) if uid_match else '',
                'internal_date': extract_imap_internaldate(head_text),
                'raw_header': raw_header,
                'response': head_text.strip(),
            })
            continue
    ...
    return records, ' '.join(fragment for fragment in response_fragments if fragment).strip()


def fetch_imap_message_header_batch(mail, message_ids: List[Any], preferred_mode: str = 'uid',
                                    allow_uid_fallback: bool = True) -> tuple[str, List[Dict[str, Any]], str, List[Dict[str, Any]], str]:
    identifier_texts = [
        message_id.decode('utf-8', errors='ignore') if isinstance(message_id, (bytes, bytearray)) else str(message_id)
        for message_id in (message_ids or [])
    ]
    if not identifier_texts:
        return 'OK', [], preferred_mode or 'uid', [], ''

    joined_identifiers = ','.join(identifier_texts)
    joined_identifier_bytes = joined_identifiers.encode('utf-8')
    modes = [preferred_mode or 'uid']
    if allow_uid_fallback and modes[0] != 'uid':
        modes.append('uid')
    if modes[0] != 'sequence':
    ...
    for index, mode in enumerate(modes):
        try:
            if mode == 'uid':
                status, data = mail.uid('FETCH', joined_identifier_bytes, IMAP_RECIPIENT_HEADER_FETCH_QUERY)
            else:
                status, data = mail.fetch(joined_identifiers, IMAP_RECIPIENT_HEADER_FETCH_QUERY)
        except Exception as exc:
            attempts.append({
                'mode': mode,
                'status': type(exc).__name__,
        ...
        records, response_text = parse_imap_header_batch_fetch_response(data)
        attempts.append({
            'mode': mode,
            'status': str(status),
            'response': sanitize_error_details(response_text or str(data or ''))[:200],
            'record_count': len(records),
        })
        if status != 'OK' and imap_fetch_attempts_include_timeout(attempts[-1:]):
            raise TimeoutError('IMAP recipient header fetch timed out')
        if status == 'OK':
            return status, records, mode, attempts, response_text
        if str(status) not in {'NO', 'BAD'} or index == len(modes) - 1:
            return str(status), records, mode, attempts, response_text
...
def imap_fetch_attempts_include_timeout(attempts: List[Dict[str, Any]]) -> bool:
    for attempt in attempts or []:
        status = str(attempt.get('status') or '').lower()
        response = str(attempt.get('response') or '').lower()
        if 'timeout' in status or 'timed out' in response or 'timeout' in response:
            return True
    return False
...
def get_emails_imap_generic_by_recipient(email_addr: str, imap_password: str, imap_host: str,
                                         imap_port: int = 993, folder: str = 'inbox',
    ...
    if not normalized_recipient:
        return {
            'success': False,
            'error': build_error_payload(
                'IMAP_RECIPIENT_INVALID',
                'recipient 涓嶈兘涓虹┖',
                'ValidationError',
                400,
                ''
            ),
            'error_code': 'IMAP_RECIPIENT_INVALID',
    ...
    try:
        ...
        scan_limit = max(1, int(scan_limit or MAILBOXES_MESSAGES_SCANNED_COUNT_DEFAULT))
        mail = create_imap_connection(imap_host, imap_port, proxy_url)
        try:
            mail.login(email_addr, imap_password)
        except imaplib.IMAP4.error as exc:
        ...
        if not selected:
            ...
            return {
                ...
                'error': build_error_payload(
                    'IMAP_FOLDER_NOT_FOUND',
                    'IMAP folder not found or inaccessible',
                    'IMAPFolderError',
                    400,
                    {
                        'provider': provider,
        ...
        message_ids, search_mode, search_attempts = search_imap_message_ids_by_recipient(mail, normalized_recipient)
        if search_mode == 'unsupported':
            return {
                'success': False,
                'error': build_error_payload(
                    'IMAP_RECIPIENT_SEARCH_UNSUPPORTED',
                    'This IMAP server does not support recipient header search',
                    'IMAPSearchUnsupported',
                    501,
                    {'attempts': search_attempts[:10]}
                ),
                'error_code': 'IMAP_RECIPIENT_SEARCH_UNSUPPORTED',
        ...
        if not message_ids:
            ...
            if not message_ids and selected_exists > 0:
                message_ids = build_sequence_message_ids(selected_exists)
                search_mode = 'sequence'
                recovery_sequence_ids_from_exists = True
        if not message_ids:
            return {
                'success': True,
                'emails': [],
        ...
        while recovery_scan_enabled and index < len(ordered_message_ids) and len(emails_data) < limit and scanned_count < scan_limit:
            ...
            try:
                batch_status, batch_records, fetch_mode, fetch_attempts, _response_text = fetch_imap_message_header_batch(
                    ...
                    batch_ids,
                    preferred_mode=search_mode or 'uid',
                    allow_uid_fallback=not recovery_sequence_ids_from_exists,
                )
            except Exception as exc:
                if is_timeout_like_exception(exc):
                    raise
            ...
            for message_id in batch_ids:
                if len(emails_data) >= limit or scanned_count >= scan_limit:
                    break
        ...
        if scanned_count >= scan_limit and index < len(ordered_message_ids) and len(emails_data) < limit:
        ...
        if is_timeout_like_exception(exc):
            timeout_result = build_email_detail_exception_result(exc)
            return {
                'success': False,
                'error': timeout_result['error'],
                'error_code': timeout_result['error']['code'],
                'recipient_search_supported': True,
            }
        return {
            'success': False,
            'error': build_error_payload(
                'IMAP_CONNECT_FAILED',
                sanitize_error_details(str(exc)) or 'IMAP 杩炴帴澶辫触',
                'IMAPConnectError',
                502,
                ''
            ),
...
def public_mailbox_request_wants_html() -> bool:
    requested_format = str(request.args.get('format') or '').strip().lower()
    return requested_format in ('', 'html')
...
def public_mailbox_api_key_error(message: str, status: int):
    if public_mailbox_request_wants_html():
        return public_mailbox_html_message_response(message, status)
...
def public_mailbox_api_key_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        g.public_mailbox_api_key_id = None
        g.public_mailbox_api_key_account_id = None
        if not is_public_mailbox_api_key_auth_enabled():
            return f(*args, **kwargs)
        if 'X-API-Key' in request.headers:
            provided = True
            candidate = normalize_api_key(request.headers.get('X-API-Key'))
        else:
            provided = 'key' in request.args
            candidate = normalize_api_key(request.args.get('key'))
        if not provided:
            return public_mailbox_api_key_error('缺少 API 密钥', 401)
        authenticated_key = (
            authenticate_public_mailbox_api_key(candidate)
            if candidate
            else None
        )
        if not authenticated_key:
            return public_mailbox_api_key_error(
                'API 密钥无效或已过期',
                403,
            )
        g.public_mailbox_api_key_id = authenticated_key['id']
        g.public_mailbox_api_key_account_id = authenticated_key.get('account_id')
        return f(*args, **kwargs)

    decorated_function._requires_public_mailbox_api_key = True
    return decorated_function

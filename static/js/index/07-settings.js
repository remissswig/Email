        ...
        let publicMailboxApiKeys = [];
        let publicMailboxApiKeyAccounts = [];
        let publicMailboxApiKeyAuthEnabled = false;
        let currentPublicMailboxApiKeySecret = '';
        let publicMailboxApiKeyLoadSequence = 0;
        let publicMailboxApiKeyAuthMutationVersion = 0;
        let publicMailboxApiKeyAuthMutationInFlight = 0;
        let publicMailboxApiKeyModalReturnFocus = null;
        let clusterNodes = [];
        let clusterPrimaryFingerprint = '';
        let clusterPrimaryCursor = 0;
        let clusterNodeLoadSequence = 0;
        let clusterNodeResultModalReturnFocus = null;
        ...
        function setPublicMailboxApiKeyStatus(message, type = '', modal = false) {
            const statusEl = document.getElementById(
                modal ? 'publicMailboxApiKeyModalStatus' : 'publicMailboxApiKeyStatus'
            );
            if (!statusEl) return;
            statusEl.textContent = message || '';
            statusEl.dataset.type = type || '';
        ...
        function syncPublicMailboxApiKeyAuthState() {
            const enabled = publicMailboxApiKeyAuthEnabled === true;
            const toggleEl = document.getElementById('publicMailboxApiKeyAuthEnabled');
            const statusEl = document.getElementById('publicMailboxApiKeyAuthStatus');
            if (toggleEl) {
                toggleEl.checked = enabled;
            }
            if (!statusEl) return;
            statusEl.dataset.enabled = enabled ? 'true' : 'false';
            const detailEl = statusEl.querySelector('div span');
            if (detailEl) {
                detailEl.textContent = enabled
                    ? '已开启：调用必须提供有效的专用 API 密钥。'
                    : '关闭时，公网访问者可能读取完整邮件正文。';
            }
        }

        async function updatePublicMailboxApiKeyAuthEnabled(enabled) {
            const toggleEl = document.getElementById('publicMailboxApiKeyAuthEnabled');
            const previous = publicMailboxApiKeyAuthEnabled;
            if (toggleEl) {
                toggleEl.disabled = true;
            }
            publicMailboxApiKeyAuthMutationVersion += 1;
            const mutationToken = publicMailboxApiKeyAuthMutationVersion;
            publicMailboxApiKeyAuthMutationInFlight = mutationToken;
            publicMailboxApiKeyAuthEnabled = Boolean(enabled);
            syncPublicMailboxApiKeyAuthState();
            try {
                const response = await fetch('/api/settings/public-mailbox-api-key-auth', {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                ...
                const data = await response.json();
                if (!response.ok || !data.success) {
                    throw new Error(data.error || '更新接口密钥校验失败');
                }
                publicMailboxApiKeyAuthEnabled = Boolean(data.auth_enabled);
                syncPublicMailboxApiKeyAuthState();
                showToast(
                    publicMailboxApiKeyAuthEnabled
                        ? '接口密钥校验已开启'
                        : '接口密钥校验已关闭',
                    'success'
                );
            } catch (error) {
                publicMailboxApiKeyAuthEnabled = previous;
                syncPublicMailboxApiKeyAuthState();
                showToast(error.message || '更新接口密钥校验失败', 'error');
            } finally {
                if (publicMailboxApiKeyAuthMutationInFlight === mutationToken) {
                    publicMailboxApiKeyAuthMutationInFlight = 0;
                }
                if (toggleEl) {
        ...
        function formatPublicMailboxApiKeyTime(value, emptyLabel) {
            return value ? formatAbsoluteDateTime(value) : emptyLabel;
        }

        function renderPublicMailboxApiKeys() {
            const listEl = document.getElementById('publicMailboxApiKeyList');
            const summaryEl = document.getElementById('publicMailboxApiKeySummary');
            if (!listEl || !summaryEl) return;

            summaryEl.textContent = `共 ${publicMailboxApiKeys.length} 个密钥`;
            if (!publicMailboxApiKeys.length) {
                listEl.innerHTML = '<tr><td class="public-mailbox-key-empty" colspan="8">尚未创建密钥。<button class="btn btn-sm btn-primary" type="button" onclick="openCreatePublicMailboxApiKeyModal()">新建密钥</button></td></tr>';
                return;
            }
            ...
            listEl.innerHTML = publicMailboxApiKeys.map(item => {
                const keyId = Number(item.id);
                const status = String(item.status || 'active');
                const statusClass = ['active', 'expiring_soon', 'expired'].includes(status)
                    ? status
                    : 'active';
                const expiryText = item.expires_at ? formatPublicMailboxApiKeyTime(item.expires_at, '永久') : '永久';
                const lastUsedText = item.last_used_at ? formatPublicMailboxApiKeyTime(item.last_used_at, '从未使用') : '从未使用';
                const remarkHtml = item.remark
                    ? `<span>${escapeHtml(item.remark)}</span>`
                    : '';
                return `
                    <tr>
                        <td><strong>${escapeHtml(item.name || '')}</strong>${remarkHtml}</td>
                        <td><code>${escapeHtml(item.masked_key || '')}</code></td>
                        <td>${escapeHtml(item.account_email || '未绑定')}</td>
                        <td><span class="public-mailbox-key-state public-mailbox-key-state--${statusClass}">${escapeHtml(statusLabels[statusClass] || statusClass)}</span></td>
                        <td>${escapeHtml(expiryText)}</td>
                        <td>${escapeHtml(formatPublicMailboxApiKeyTime(item.created_at, '-'))}</td>
                        <td>${escapeHtml(lastUsedText)}</td>
                        <td>
                            <div class="public-mailbox-key-actions">
                                <button class="btn btn-sm btn-secondary" type="button" onclick="revealPublicMailboxApiKey(${keyId})">查看</button>
                                <button class="btn btn-sm btn-secondary" type="button" onclick="copyPublicMailboxApiKey(${keyId})">复制</button>
                                <button class="btn btn-sm btn-danger" type="button" onclick="deletePublicMailboxApiKey(${keyId})">删除</button>
                            </div>
                        </td>
                    </tr>
                `;
        ...
        function renderPublicMailboxApiKeyAccountOptions() {
            const accountSelect = document.getElementById('publicMailboxApiKeyAccount');
            if (!accountSelect) return;
            const selectedValue = accountSelect.value;
            accountSelect.innerHTML = '<option value="">不绑定（兼容旧模式）</option>'
                + publicMailboxApiKeyAccounts.map(account => (
                    `<option value="${Number(account.id)}">${escapeHtml(account.email || '')}</option>`
                )).join('');
            if ([...accountSelect.options].some(option => option.value === selectedValue)) {
                accountSelect.value = selectedValue;
            }
        }

        async function loadPublicMailboxApiKeys() {
            const loadSequence = ++publicMailboxApiKeyLoadSequence;
            const authMutationVersionAtStart = publicMailboxApiKeyAuthMutationVersion;
            const authMutationInFlightAtStart = publicMailboxApiKeyAuthMutationInFlight;
            setPublicMailboxApiKeyStatus('正在加载...', 'pending');
            try {
                const response = await fetch('/api/settings/public-mailbox-api-keys', { cache: 'no-store' });
                const data = await response.json();
                if (!response.ok || !data.success) {
                    throw new Error(data.error || '加载 API 密钥失败');
                }
                if (loadSequence !== publicMailboxApiKeyLoadSequence) return;

                publicMailboxApiKeys = Array.isArray(data.keys)
                    ? data.keys.map(key => ({
                        id: key.id,
                        name: key.name || '',
                        remark: key.remark || '',
                    ...
                    : [];
                publicMailboxApiKeyAccounts = Array.isArray(data.accounts)
                    ? data.accounts.map(account => ({
                        id: account.id,
                        email: account.email || ''
                    }))
                    : [];
                renderPublicMailboxApiKeyAccountOptions();
                if (authMutationInFlightAtStart === 0
                    && publicMailboxApiKeyAuthMutationInFlight === 0
                    && authMutationVersionAtStart === publicMailboxApiKeyAuthMutationVersion) {
                    publicMailboxApiKeyAuthEnabled = Boolean(data.auth_enabled);
                    syncPublicMailboxApiKeyAuthState();
                }
                renderPublicMailboxApiKeys();
                setPublicMailboxApiKeyStatus('');
            } catch (error) {
                if (loadSequence !== publicMailboxApiKeyLoadSequence) return;
                const listEl = document.getElementById('publicMailboxApiKeyList');
                const summaryEl = document.getElementById('publicMailboxApiKeySummary');
                publicMailboxApiKeys = [];
                if (summaryEl) {
                    summaryEl.textContent = '加载失败';
                }
                if (listEl) {
                    listEl.innerHTML = '<tr><td class="public-mailbox-key-empty" colspan="8">加载 API 密钥失败。<button class="btn btn-sm btn-secondary" type="button" onclick="loadPublicMailboxApiKeys()">重试</button></td></tr>';
                }
                setPublicMailboxApiKeyStatus((error.message || '加载 API 密钥失败') + '，请重试。', 'error');
            }
        }

        function syncPublicMailboxApiKeyCreationMode() {
            const mode = document.querySelector('input[name="publicMailboxApiKeyMode"]:checked')?.value || 'generated';
            const customGroup = document.getElementById('publicMailboxApiKeyCustomSecretGroup');
            if (customGroup) {
                customGroup.hidden = mode !== 'custom';
            }
        }

        function syncPublicMailboxApiKeyExpiryFields() {
            const expiry = document.getElementById('publicMailboxApiKeyExpiry')?.value || 'permanent';
            const customGroup = document.getElementById('publicMailboxApiKeyCustomExpiryGroup');
            if (customGroup) {
                customGroup.hidden = expiry !== 'custom';
            }
        }

        function togglePublicMailboxApiKeyCustomSecretVisibility() {
            const input = document.getElementById('publicMailboxApiKeyCustomSecret');
            const toggleBtn = document.getElementById('publicMailboxApiKeyCustomSecretToggle');
        ...
        function showPublicMailboxApiKeyModal() {
            const modal = document.getElementById('publicMailboxApiKeyModal');
            if (!modal?.classList.contains('show')) {
                publicMailboxApiKeyModalReturnFocus = document.activeElement;
            }
            setModalVisible('publicMailboxApiKeyModal', true);
        }

        function resetPublicMailboxApiKeyModal() {
            const titleEl = document.getElementById('publicMailboxApiKeyModalTitle');
            const nameInput = document.getElementById('publicMailboxApiKeyName');
            const remarkInput = document.getElementById('publicMailboxApiKeyRemark');
            const accountSelect = document.getElementById('publicMailboxApiKeyAccount');
            const generatedRadio = document.getElementById('publicMailboxApiKeyModeGenerated');
            const customRadio = document.getElementById('publicMailboxApiKeyModeCustom');
            const secretInput = document.getElementById('publicMailboxApiKeySecret');
            const hintEl = document.getElementById('publicMailboxApiKeySecretHint');
            const customSecretInput = document.getElementById('publicMailboxApiKeyCustomSecret');
            const toggleBtn = document.getElementById('publicMailboxApiKeyCustomSecretToggle');
            const expirySelect = document.getElementById('publicMailboxApiKeyExpiry');
            const customExpiryInput = document.getElementById('publicMailboxApiKeyCustomExpiry');
            const formPanel = document.getElementById('publicMailboxApiKeyFormPanel');
            const secretPanel = document.getElementById('publicMailboxApiKeySecretPanel');
            const cancelBtn = document.getElementById('publicMailboxApiKeyCancelBtn');
            const doneBtn = document.getElementById('publicMailboxApiKeyDoneBtn');
            const createBtn = document.getElementById('createPublicMailboxApiKeyBtn');

            if (titleEl) titleEl.textContent = '新建 API 密钥';
            ...
            if (customSecretInput) {
                customSecretInput.value = '';
                customSecretInput.type = 'password';
            }
            if (toggleBtn) {
            ...
            currentPublicMailboxApiKeySecret = '';
            setPublicMailboxApiKeyStatus('', '', true);
            syncPublicMailboxApiKeyCreationMode();
            syncPublicMailboxApiKeyExpiryFields();
        }

        function openCreatePublicMailboxApiKeyModal() {
            resetPublicMailboxApiKeyModal();
            showPublicMailboxApiKeyModal();
        }

        function restorePublicMailboxApiKeyModalFocus() {
            const modal = document.getElementById('publicMailboxApiKeyModal');
            if (!modal?.classList.contains('show')) {
                publicMailboxApiKeyModalReturnFocus = null;
                return;
            }

            const returnFocus = publicMailboxApiKeyModalReturnFocus;
        ...
        function hidePublicMailboxApiKeyModal() {
            resetPublicMailboxApiKeyModal();
            restorePublicMailboxApiKeyModalFocus();
            hideModal('publicMailboxApiKeyModal');
        }

        function buildPublicMailboxApiKeyExpiry() {
            const expiry = document.getElementById('publicMailboxApiKeyExpiry')?.value || 'permanent';
            ...
            if (expiry === 'custom') {
                const customInput = document.getElementById('publicMailboxApiKeyCustomExpiry');
                const customValue = customInput?.value || '';
                if (!customValue) {
                    throw new Error('请选择自定义到期时间');
                }
        ...
        function showPublicMailboxApiKeySecret(name, secret, created = false) {
            const titleEl = document.getElementById('publicMailboxApiKeyModalTitle');
            const hintEl = document.getElementById('publicMailboxApiKeySecretHint');
            const secretInput = document.getElementById('publicMailboxApiKeySecret');
            const formPanel = document.getElementById('publicMailboxApiKeyFormPanel');
            ...
            resetPublicMailboxApiKeyModal();
            currentPublicMailboxApiKeySecret = secret || '';
            if (titleEl) {
                titleEl.textContent = created ? '密钥已创建' : '查看 API 密钥';
            }
            if (hintEl) {
                hintEl.textContent = created
                    ? `请立即复制并妥善保存“${name || 'API 密钥'}”。关闭后仍可通过查看重新获取。`
                    : `“${name || 'API 密钥'}”的完整密钥`;
            }
            if (secretInput) {
        ...
        function handlePublicMailboxApiKeyModalEscape(event) {
            const modal = document.getElementById('publicMailboxApiKeyModal');
        ...
        document.addEventListener('keydown', handlePublicMailboxApiKeyModalEscape);

        async function createPublicMailboxApiKey() {
            const nameInput = document.getElementById('publicMailboxApiKeyName');
            const remarkInput = document.getElementById('publicMailboxApiKeyRemark');
            const accountSelect = document.getElementById('publicMailboxApiKeyAccount');
            const createBtn = document.getElementById('createPublicMailboxApiKeyBtn');
            const customSecretInput = document.getElementById('publicMailboxApiKeyCustomSecret');
            if (!nameInput || !remarkInput || !accountSelect || !createBtn || !customSecretInput) return;

            const name = nameInput.value.trim();
            const remark = remarkInput.value.trim();
            if (!name) {
                setPublicMailboxApiKeyStatus('请输入密钥名称', 'error', true);
                return;
            }

            const mode = document.querySelector('input[name="publicMailboxApiKeyMode"]:checked')?.value || 'generated';
            const customSecret = customSecretInput.value.trim();
            if (mode === 'custom') {
                if (!customSecret || customSecret.length > 512) {
                    setPublicMailboxApiKeyStatus('自定义密钥长度必须为 1 到 512 个字符', 'error', true);
                    return;
                }
            }
            ...
            let expiresAt = null;
            try {
                expiresAt = buildPublicMailboxApiKeyExpiry();
            } catch (error) {
                setPublicMailboxApiKeyStatus(error.message || '到期时间无效', 'error', true);
                return;
            }

            createBtn.disabled = true;
            setPublicMailboxApiKeyStatus('正在创建...', 'pending', true);
            try {
                const response = await fetch('/api/settings/public-mailbox-api-keys', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        name: name,
                        ...
                        secret: mode === 'custom' ? customSecret : null,
                ...
                currentPublicMailboxApiKeySecret = data.key.secret || '';
                showPublicMailboxApiKeySecret(data.key.name || name, data.key.secret || '', true);
                await loadPublicMailboxApiKeys();
            } catch (error) {
                setPublicMailboxApiKeyStatus(error.message || '创建 API 密钥失败', 'error', true);
            } finally {
                createBtn.disabled = false;
            }
        }

        async function fetchPublicMailboxApiKeySecret(keyId) {
            const response = await fetch(`/api/settings/public-mailbox-api-keys/${encodeURIComponent(keyId)}/secret`, {
                cache: 'no-store'
            });
            const data = await response.json();
            if (!response.ok || !data.success) {
                throw new Error(data.error || '读取 API 密钥失败');
            }
            return data.secret || '';
        }

        async function revealPublicMailboxApiKey(keyId) {
            const item = publicMailboxApiKeys.find(candidate => Number(candidate.id) === Number(keyId));
            try {
                const secret = await fetchPublicMailboxApiKeySecret(keyId);
                showPublicMailboxApiKeySecret(item?.name || 'API 密钥', secret, false);
            } catch (error) {
                showToast(error.message || '读取 API 密钥失败', 'error');
            }
        }

        async function copyPublicMailboxApiKey(keyId) {
            try {
                const secret = await fetchPublicMailboxApiKeySecret(keyId);
                await copyTextToClipboard(secret, 'API 密钥已复制');
            } catch (error) {
                showToast(error.message || '复制 API 密钥失败', 'error');
            }
        }

        function copyCurrentPublicMailboxApiKeySecret() {
            if (!currentPublicMailboxApiKeySecret) return;
            copyTextToClipboard(currentPublicMailboxApiKeySecret, 'API 密钥已复制');
        }

        async function deletePublicMailboxApiKey(keyId) {
            const item = publicMailboxApiKeys.find(candidate => Number(candidate.id) === Number(keyId));
            const name = item?.name || '该密钥';
            const confirmed = await showConfirmModal(
                `确定要删除“${name}”吗？删除后调用将立即失效。`,
                { title: '删除 API 密钥', confirmText: '确认删除' }
            );
            if (!confirmed) return;

            try {
                const response = await fetch(`/api/settings/public-mailbox-api-keys/${encodeURIComponent(keyId)}`, {
                    method: 'DELETE'
                });
                const data = await response.json();
                if (!response.ok || !data.success) {
                ...
                showToast(error.message || '删除 API 密钥失败', 'error');
            }
        }

        function setClusterNodeStatus(message, type = '') {
            const statusEl = document.getElementById('clusterNodeStatus');
            if (!statusEl) return;
            statusEl.textContent = message || '';
            statusEl.dataset.type = type || '';
            statusEl.hidden = !message;
        ...
        function formatClusterNodeTime(value, emptyLabel = '-') {
            return value ? formatAbsoluteDateTime(value) : emptyLabel;
        }

        function buildClusterNodeInstallCommand(nodeId, primaryFingerprint) {
            const masterUrl = window.location.origin;
            return `bash <(curl -Ls https://raw.githubusercontent.com/seldom1024/email-scripts/refs/heads/master/install-node.sh) --master ${masterUrl} --node-id ${nodeId} --master-fingerprint ${primaryFingerprint}`;
        }

        function resetClusterNodeResultState() {
            const tokenInput = document.getElementById('clusterNodeResultValue');
            const commandInput = document.getElementById('clusterNodeInstallCommand');
            if (tokenInput) {
                tokenInput.value = '';
            }
            if (commandInput) {
        ...
        function showClusterNodeResultModal() {
            const modal = document.getElementById('clusterNodeResultModal');
            if (!modal?.classList.contains('show')) {
                clusterNodeResultModalReturnFocus = document.activeElement;
            }
            setModalVisible('clusterNodeResultModal', true);
        }

        function restoreClusterNodeResultModalFocus() {
            const modal = document.getElementById('clusterNodeResultModal');
            if (!modal?.classList.contains('show')) {
                clusterNodeResultModalReturnFocus = null;
                return;
            }

            const returnFocus = clusterNodeResultModalReturnFocus;
            const settingsModal = document.getElementById('settingsModal');
            clusterNodeResultModalReturnFocus = null;
            if (returnFocus instanceof HTMLElement
                && returnFocus.isConnected
        ...
        function hideClusterNodeResultModal() {
            resetClusterNodeResultState();
            restoreClusterNodeResultModalFocus();
            hideModal('clusterNodeResultModal');
        }

        function handleClusterNodeResultModalEscape(event) {
            const modal = document.getElementById('clusterNodeResultModal');
            if (event.key !== 'Escape' || !modal?.classList.contains('show')) return;
            event.preventDefault();
            event.stopImmediatePropagation();
        ...
        document.addEventListener('keydown', handleClusterNodeResultModalEscape);

        function openCreateClusterNodeModal() {
            const nameInput = document.getElementById('clusterNodeName');
            const remarkInput = document.getElementById('clusterNodeRemark');
            if (nameInput) {
                nameInput.value = '';
                nameInput.focus();
            }
            ...
            setClusterNodeStatus('');
        }

        function renderClusterNodes() {
            const listEl = document.getElementById('clusterNodeList');
            const summaryEl = document.getElementById('clusterNodeSummary');
            if (!listEl || !summaryEl) return;

            summaryEl.textContent = `共 ${clusterNodes.length} 个节点 · 主游标 ${clusterPrimaryCursor}`;
            if (!clusterNodes.length) {
                listEl.innerHTML = '<tr><td class="cluster-node-empty" colspan="9">尚未创建节点。<button class="btn btn-sm btn-primary" type="button" onclick="openCreateClusterNodeModal()">新建节点</button></td></tr>';
                return;
            }
            ...
            listEl.innerHTML = clusterNodes.map(node => {
                const nodeId = escapeHtml(node.id || '');
                const status = ['pending', 'active', 'revoked'].includes(node.status) ? node.status : 'pending';
                const displayStatus = node.display_status || node.status || 'pending';
                const displayStatusClass = ['pending', 'synchronizing', 'online', 'offline', 'expired', 'revoked'].includes(displayStatus)
                    ? displayStatus
                    : 'pending';
                const lagValue = Math.max(clusterPrimaryCursor - Number(node.last_ack_cursor || 0), 0);
                const remarkHtml = node.remark ? `<span>${escapeHtml(node.remark)}</span>` : '';
                const versionParts = [];
                if (node.replica_app_version) {
                    versionParts.push(`App ${escapeHtml(node.replica_app_version)}`);
                }
                if (node.replica_protocol_version !== null && node.replica_protocol_version !== undefined && node.replica_protocol_version !== '') {
                    versionParts.push(`Protocol ${escapeHtml(String(node.replica_protocol_version))}`);
                }
                const versionHtml = versionParts.length ? versionParts.join('<br>') : '-';
                const actionButtons = [];
                if (status === 'pending') {
                    actionButtons.push(`<button class="btn btn-sm btn-secondary" type="button" onclick="issueClusterNodeToken('${nodeId}')">查看令牌</button>`);
                    actionButtons.push(`<button class="btn btn-sm btn-danger" type="button" onclick="revokeClusterNode('${nodeId}')">撤销</button>`);
                }
                if (status === 'active') {
                    actionButtons.push(`<button class="btn btn-sm btn-secondary" type="button" onclick="rotateClusterNodeCredential('${nodeId}')">轮换凭据</button>`);
                    actionButtons.push(`<button class="btn btn-sm btn-danger" type="button" onclick="revokeClusterNode('${nodeId}')">撤销</button>`);
                }
                if (status === 'revoked') {
                    actionButtons.push(`<button class="btn btn-sm btn-danger" type="button" onclick="deleteRevokedClusterNode('${nodeId}')">删除</button>`);
                }
                return `
                    <tr>
                        <td><span class="cluster-node-status cluster-node-status--${displayStatusClass}">${escapeHtml(statusLabels[displayStatusClass] || displayStatusClass)}</span></td>
                        <td><strong>${escapeHtml(node.name || '')}</strong>${remarkHtml}</td>
                        <td>${versionHtml}</td>
                        <td>${escapeHtml(formatClusterNodeTime(node.last_sync_at, '从未同步'))}</td>
                        <td><code>${escapeHtml(String(node.last_ack_cursor || 0))}</code></td>
                        <td><code>${escapeHtml(String(lagValue))}</code></td>
                        <td>${escapeHtml(node.last_error_sanitized || '-')}</td>
                        <td><code>${escapeHtml(node.source_ip || '-')}</code></td>
                        <td><div class="cluster-node-actions">${actionButtons.join('')}</div></td>
                    </tr>
                `;
            }).join('');
        }
        ...
        async function loadClusterNodes() {
            const loadSequence = ++clusterNodeLoadSequence;
            setClusterNodeStatus('正在加载...', 'pending');
            try {
                const response = await fetch('/api/settings/cluster/nodes', { cache: 'no-store' });
                const data = await response.json();
                if (!response.ok || !data.success) {
                    throw new Error(data.error || '加载副本节点失败');
                }
                if (loadSequence !== clusterNodeLoadSequence) return;

                clusterPrimaryFingerprint = data.primary_fingerprint || '';
                clusterPrimaryCursor = Number(data.primary_cursor || 0);
                clusterNodes = Array.isArray(data.nodes)
                    ? data.nodes.map(node => ({
                        id: node.id || '',
                        name: node.name || '',
                        remark: node.remark || '',
                        ...
                        last_ack_cursor: Number(node.last_ack_cursor || 0),
                        credential_version: Number(node.credential_version || 0),
                        display_status: node.display_status || '',
                        replica_app_version: node.replica_app_version || '',
                        replica_protocol_version: node.replica_protocol_version,
                        last_error_sanitized: node.last_error_sanitized || ''
                    }))
                    : [];
                renderClusterNodes();
                setClusterNodeStatus('');
                ...
                if (listEl) {
                    listEl.innerHTML = '<tr><td class="cluster-node-empty" colspan="9">加载副本节点失败。<button class="btn btn-sm btn-secondary" type="button" onclick="loadClusterNodes()">重试</button></td></tr>';
                }
                setClusterNodeStatus((error.message || '加载副本节点失败') + '，请重试。', 'error');
            }
        }

        function showClusterNodeEnrollmentResult(node, enrollmentToken, primaryFingerprint) {
            resetClusterNodeResultState();
            const tokenInput = document.getElementById('clusterNodeResultValue');
            const commandInput = document.getElementById('clusterNodeInstallCommand');
            if (tokenInput) {
                tokenInput.value = enrollmentToken || '';
            }
            if (commandInput) {
                commandInput.value = buildClusterNodeInstallCommand(node?.id || '', primaryFingerprint || clusterPrimaryFingerprint || '');
            }
            showClusterNodeResultModal();
        }

        function copyClusterNodeResultValue() {
            const token = document.getElementById('clusterNodeResultValue')?.value || '';
            if (!token) return;
            copyTextToClipboard(token, '一次性令牌已复制');
        }

        function copyClusterNodeInstallCommand() {
            const command = document.getElementById('clusterNodeInstallCommand')?.value || '';
            if (!command) return;
            copyTextToClipboard(command, '安装命令已复制');
        }

        async function createClusterNode() {
            const nameInput = document.getElementById('clusterNodeName');
            const remarkInput = document.getElementById('clusterNodeRemark');
            const createBtn = document.getElementById('createClusterNodeBtn');
            if (!nameInput || !remarkInput || !createBtn) return;

            const name = nameInput.value.trim();
            const remark = remarkInput.value.trim();
            if (!name) {
                setClusterNodeStatus('请输入节点名称', 'error');
                return;
            }

            createBtn.disabled = true;
            setClusterNodeStatus('正在创建...', 'pending');
            try {
                const response = await fetch('/api/settings/cluster/nodes', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                const data = await response.json();
                if (!response.ok || !data.success) {
                    throw new Error(data.error || '创建节点失败');
                }
                nameInput.value = '';
                remarkInput.value = '';
                await loadClusterNodes();
                setClusterNodeStatus('节点已创建', 'success');
                showClusterNodeEnrollmentResult(data.node || {}, data.enrollment_token || '', data.primary_fingerprint || '');
            } catch (error) {
                setClusterNodeStatus(error.message || '创建节点失败', 'error');
            } finally {
                createBtn.disabled = false;
            }
        }

        async function issueClusterNodeToken(nodeId) {
            setClusterNodeStatus('正在生成一次性令牌...', 'pending');
            try {
                const response = await fetch(`/api/settings/cluster/nodes/${encodeURIComponent(nodeId)}/token`, {
                    method: 'POST'
                });
                const data = await response.json();
                if (!response.ok || !data.success) {
                    throw new Error(data.error || '生成一次性令牌失败');
                }
                setClusterNodeStatus('一次性令牌已更新', 'success');
                showClusterNodeEnrollmentResult(data.node || {}, data.enrollment_token || '', data.primary_fingerprint || '');
            } catch (error) {
                setClusterNodeStatus(error.message || '生成一次性令牌失败', 'error');
            }
        }

        async function rotateClusterNodeCredential(nodeId) {
            setClusterNodeStatus('正在轮换凭据...', 'pending');
            try {
                const response = await fetch(`/api/settings/cluster/nodes/${encodeURIComponent(nodeId)}/rotate`, {
                    method: 'POST'
                });
                const data = await response.json();
                if (!response.ok || !data.success) {
                    throw new Error(data.error || '轮换节点凭据失败');
                }
                await loadClusterNodes();
                showToast('节点凭据已轮换', 'success');
            } catch (error) {
                setClusterNodeStatus(error.message || '轮换节点凭据失败', 'error');
                showToast(error.message || '轮换节点凭据失败', 'error');
            }
        }

        async function revokeClusterNode(nodeId) {
            const node = clusterNodes.find(candidate => candidate.id === nodeId);
            const confirmed = await showConfirmModal(
                `确定要撤销“${node?.name || nodeId}”吗？撤销后该节点将不能继续同步。`,
                { title: '撤销副本节点', confirmText: '确认撤销' }
            );
            if (!confirmed) return;

            setClusterNodeStatus('正在撤销节点...', 'pending');
            try {
                const response = await fetch(`/api/settings/cluster/nodes/${encodeURIComponent(nodeId)}/revoke`, {
                    method: 'POST'
                });
                const data = await response.json();
                if (!response.ok || !data.success) {
                    throw new Error(data.error || '撤销副本节点失败');
                }
                await loadClusterNodes();
                showToast('副本节点已撤销', 'success');
            } catch (error) {
                setClusterNodeStatus(error.message || '撤销副本节点失败', 'error');
                showToast(error.message || '撤销副本节点失败', 'error');
            }
        }

        async function deleteRevokedClusterNode(nodeId) {
            const node = clusterNodes.find(candidate => candidate.id === nodeId);
            if (node?.status !== 'revoked') {
                setClusterNodeStatus('只有已撤销的节点才能删除', 'error');
                return;
            }

            const confirmed = await showConfirmModal(
                `确定要删除“${node.name || nodeId}”吗？删除后需要重新创建并注册。`,
                { title: '删除副本节点', confirmText: '确认删除' }
            );
            if (!confirmed) return;

            setClusterNodeStatus('正在删除节点...', 'pending');
            try {
                const response = await fetch(`/api/settings/cluster/nodes/${encodeURIComponent(nodeId)}`, {
                    method: 'DELETE'
                });
                const data = await response.json();
                if (!response.ok || !data.success) {
                    throw new Error(data.error || '删除副本节点失败');
                }
                await loadClusterNodes();
                showToast('副本节点已删除', 'success');
            } catch (error) {
                setClusterNodeStatus(error.message || '删除副本节点失败', 'error');
                showToast(error.message || '删除副本节点失败', 'error');
            }
        }
        ...
        async function loadSettings() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    document.getElementById('settingsMailboxesMessagesScannedCount').value = data.settings.mailboxes_messages_scanned_count || 100;
        ...
        async function saveSettings() {
            ...
            const mailboxesMessagesScannedCount = Number(document.getElementById('settingsMailboxesMessagesScannedCount').value);
            ...
            if (!Number.isInteger(mailboxesMessagesScannedCount) || mailboxesMessagesScannedCount < 1 || mailboxesMessagesScannedCount > 10000) {
                showToast('最多扫描邮件数必须是 1 到 10000 之间的整数', 'error');
                return;
            }
            ...
            settings.mailboxes_messages_scanned_count = mailboxesMessagesScannedCount;

        ...
        function getDefaultNormalGroupId() {
            const currentGroup = groups.find(group => Number(group.id) === Number(currentGroupId));
            if (currentGroup && !isSystemGroup(currentGroup)) {
                return Number(currentGroup.id);
            }
            const defaultGroup = groups.find(group => Number(group.id) === 1 && !isSystemGroup(group));
            const firstNormalGroup = groups.find(group => !isSystemGroup(group));
            const targetGroup = defaultGroup || firstNormalGroup;
            return targetGroup ? Number(targetGroup.id) : null;
        }

        async function switchToOutlookMailboxChannel() {
            const targetGroupId = getDefaultNormalGroupId();
            if (!targetGroupId) {
            ...
            localStorage.setItem('outlook_temp_email_filter', 'all');
            document.querySelectorAll('.provider-filter-btn').forEach(btn => {
                btn.classList.toggle('active', btn.dataset.provider === 'outlook');
            });
            currentGroupId = targetGroupId;
            localStorage.setItem('outlook_last_group_id', String(targetGroupId));
        ...
        function updateImportHint() {
            ...
            const isOutlookChannel = isTempGroup && channel === 'outlook';
            ...
            accountDefaultFields.forEach(field => {
                ...
                field.style.display = isTempGroup && !isOutlookChannel ? (isTagField ? '' : 'none') : '';
            });
            ...
            if (isTempGroup) {
                ...
                if (channel === 'outlook') {
                    hintEl.textContent = 'Outlook 会作为正式邮箱导入到普通分组，支持自动识别：邮箱----密码----client_id----refresh_token 或 邮箱----密码----refresh_token----client_id。';
                    inputEl.placeholder = '邮箱----密码----client_id----refresh_token';
                    if (exampleEl) {
                        exampleEl.style.display = '';
                        exampleEl.textContent = '示例：\nuser@outlook.com----password123----24d9a0ed-8787-4584-883c-2fd79308940a----0.AXEA...\nuser@hotmail.com----password123----0.AXEA...----24d9a0ed-8787-4584-883c-2fd79308940a';
                    }
                    return;
                }
                if (channel === 'duckmail') {
